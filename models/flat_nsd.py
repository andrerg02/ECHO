...
from models.adgn import AntiSymmetricConv
...
class FlatBundleConv(MessagePassing):
    ...
    def __init__(self,
        ...
        # self.ffn1 = nn.Linear(self.out_channels, self.out_channels)
        # self.ffn2 = nn.Linear(self.out_channels, self.out_channels)
    ...
    def forward(self, x: Tensor, edge_index: Tensor, data=None, reff=False):
        ...
        # x = self.bn(x)
        # #x = x.view(self.graph_size, self.d, self.hidden_channels)

        # x = x.view(self.graph_size, self.d, self.out_channels)
        # xffn = self.ffn2(F.gelu(self.ffn1(x)))
        # xffn = xffn.view(-1, self.out_channels)
        # x = x.view(-1, self.out_channels)
        # x = self.bn2(xffn + x)
    ...
    def gnn_builder(self, gnn_type, in_channels, out_channels, num_layers, hidden_channels=None):
        ...
        if hidden_channels is None or num_layers == 1:
            if gnn_type == 'GPS':
                ...
                for i in range(num_layers):
                    layers.append(gnn(in_channels, gamma=self.gamma, epsilon=self.epsilon))
            elif gnn_type == 'GIN':
                for i in range(num_layers):
                    layers.append(gnn(nn.Sequential(nn.Linear(in_channels, out_channels,nn.BatchNorm1d(out_channels), nn.ReLU(),nn.Linear(out_channels, out_channels)))), train_eps=True)
            else:
